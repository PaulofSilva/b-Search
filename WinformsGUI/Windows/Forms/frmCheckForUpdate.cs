    ...
    public partial class frmCheckForUpdate : BaseForm
    {
        ...
        private void CheckForUpdate()
        {
            try
            {
                ...
                using (WebResponse webRes = webReq.GetResponse())
                {
                    ...
                    // current version
                    Version currentVersion = bSearch.Common.ProductInformation.ApplicationVersion;

                    //if (currentVersion.CompareTo(newestVersion) < 0)
                    //{
                    GetLatestChangeLog();
                    UpdateMessage(string.Format("A newer version of bSearch is available: {0}", newestVersion.ToString(3)), newestVersion.ToString(3));
                    //}
                    //else
                    //{
        ...
        private void GetLatestChangeLog()
        {
            try
            {
                ...
                WebRequest webReq = WebRequest.Create("http://bsoftware.pt");
                webReq.Proxy = WebRequest.GetSystemWebProxy();
                webReq.Timeout = 4000;
            ...
            try
            {
                if (btnNext.Text == "Finish")
                {
                    ...
                    if (File.Exists(installFile) && (new FileInfo(installFile).Length > 0))
                    {
                        ...
                        // close all bSearch instances but our own
                ...
                else
                {
                    ...
                    string file = string.Format("bSearch_Setup_v{0}.exe", newestVersion);
                    string remoteFile = string.Format("http://downloads.biglevel.pt/bSearch/{0}", file);
