REM  This bundle script will combine all bSearch's dependency assemblies from mono.  This is done so that you can 
REM  build an bSearch executable that can run without installing .net framwork or mono framework.
REM
REM  This script requires that you have Mono2.4 installed to c:\mono-2.4\  (http://www.go-mono.com/mono-downloads/download.html)
REM  also you should have upx.exe in your path (http://bsoftware.pt/).  Upx is optional but nice because the bundled exe
...
cd %OutputDir%
call mkbundle -o %ExeName% %BinDirectory%\%ExeName%.exe %BinDirectory%\libbSearch.dll --deps
